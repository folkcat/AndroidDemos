...
public class TinyHttpUtil {
    ...
    private String urlEncodetoBGK(String text) {
        try {
     ...
     * 发送Get数据，只返回头部
     */
    public Map<String,List> getGetHeaders(String url, Map<String, String> params,
                                          boolean isAutoDirect, Map<String, String> headers) {
        Map retHeaders=null;
        try {
            String urlNameString;
     ...
     * 发送Post数据，只返回头部
     */
    public Map<String,List> getPostHeaders(String _url, Map<String, String> _params,boolean _isAutoDirect,
        ...
        try {
            ...
            conn.setDoOutput(true);
            conn.setDoInput(true);
            conn.setInstanceFollowRedirects(_isAutoDirect);//是否自动跳转
            conn.connect();
            if (_params != null) {
                out = new PrintWriter(conn.getOutputStream());
                Object[] keyArr = _params.keySet().toArray();
                String andChar;
                for (int i = 0; i < keyArr.length; i++) {
                    if (i == keyArr.length - 1) {
                        andChar = "";
	 ...
	 */
    public String getPostHeadersAndContent(String _url,
	...
	*/
	...
	/*  http://localhost/test/postandget.php 内容
		<?php
			error_reporting(0);
			echo "//POST:".$_POST['post'];
			echo "//GET:".$_GET['get'];
			echo "//HEAD:".$_SERVER['HTTP_TESTHEAD'];
		?>
	 */
